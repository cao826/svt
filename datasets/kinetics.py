...
class Kinetics(torch.utils.data.Dataset):
        ...
        for i_try in range(self._num_retries):
            ...
            if self.mode in ["test", "val"] or self.cfg.DATA.NO_RGB_AUG:
                ...
                frames = augmentation(
                    frames,
                    from_list=True,
                    no_aug=self.cfg.DATA.NO_SPATIAL,
                    two_token=self.cfg.MODEL.TWO_TOKEN,
                )
    ...
    def get_flow_from_folder(dir_path):
        ...
        flow_image_list = [
            torchvision.transforms.functional.to_tensor(im_path)
            for im_path in flow_image_list
        ]
