...
from eval_knn import (
    extract_features,
    knn_classifier,
    UCFReturnIndexDataset,
    HMDBReturnIndexDataset,
)
...
def get_args_parser():
    ...
    parser.add_argument(
        "--pretrained_rgb", default=None, type=str, help="Path to pretrained RGB model."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--opts",
        help="See utils/defaults.py for all options",
        default=None,
    ...
    # online knn eval
    parser.add_argument(
        "--eval_batch_size_per_gpu", default=128, type=int, help="Per-GPU batch-size"
    )
    parser.add_argument(
        "--nb_knn",
        default=5,
        type=int,
        help="Number of NN to use. We use 5 for online.",
    )
    parser.add_argument(
        "--temperature",
        default=0.07,
...
def train_svt(args):
    ...
    dataset = Kinetics(
        cfg=config, mode="train", num_retries=10, get_flow=config.DATA.USE_FLOW
    )
    ...
    if config.MODEL.CNN_DISTILL:
        ...
        cnn_model = nn.parallel.DistributedDataParallel(
            cnn_model, device_ids=[args.gpu], find_unused_parameters=False
        )
    else:
        cnn_model = None
    ...
    if config.MODEL.TWO_STREAM:
        if utils.has_batchnorms(motion_student):
            ...
            motion_teacher = nn.parallel.DistributedDataParallel(
                motion_teacher, device_ids=[args.gpu]
            )
        ...
        motion_student = nn.parallel.DistributedDataParallel(
            motion_student, device_ids=[args.gpu]
        )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            args,
            cfg=config,
            motion_loss=dino_flow_loss,
            cross_loss=dino_cross_loss,
            motion_student=motion_student,
            motion_teacher=motion_teacher,
            motion_teacher_without_ddp=motion_teacher_without_ddp,
            rand_conv=rand_conv,
        )
        ...
        save_dict = {
            ...
            "motion_student": motion_student.state_dict()
            if motion_student is not None
            else 0,
            "motion_teacher": motion_teacher.state_dict()
...
def train_one_epoch(
    ...
    for it, (images, _, _, meta) in enumerate(
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            if cfg.MODEL.TWO_STREAM:
                ...
                loss = (
                    dino_loss(student_output_rgb, teacher_output_rgb, epoch)
                ...
                student_output = student(
                    images[2:]
                )  # 2 spatially local and 2 temporally global local views
                teacher_output = teacher(
                    images[:2]
                )  # only 2 global views through the teacher
        ...
        with torch.no_grad():
            ...
            if cfg.MODEL.TWO_STREAM:
                for param_q, param_k in zip(
                    motion_student.module.parameters(),
...
class DINOLoss(nn.Module):
    ...
    def forward(self, student_output, teacher_output, epoch):
        ...
        if self.two_token:
            ...
            teacher_out = [
                F.softmax((x - self.center[idx]) / temp, dim=-1)
                for idx, x in enumerate(teacher_output)
            ]
    ...
    def update_center(self, teacher_output):
        ...
        if isinstance(teacher_output, (tuple, list)):
            ...
            batch_center = [
                x / (len(teacher_output[0]) * dist.get_world_size())
                for x in batch_center
            ]
            self.center[0, :] = self.center[0, :] * self.center_momentum + batch_center[
                0
            ] * (1 - self.center_momentum)
            self.center[1, :] = self.center[1, :] * self.center_momentum + batch_center[
                1
            ] * (1 - self.center_momentum)
        else:
